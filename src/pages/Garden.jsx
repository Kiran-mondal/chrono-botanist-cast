...
const SEEDS = [
  ...
  { id: 'prism', name: 'Prism Orchid', effect: 'REFRACT', glyph: '💎', note: 'Bends and redirects active time-fields around corners.', tone: 'hybrid' },
  { id: 'eclipse', name: 'Eclipse Moss', effect: 'SHIELD', glyph: '🌑', note: 'Absorbs ambient radiation, defending fragile seed pods from temporal storms.', tone: 'calm' },
...
const SECTORS = [
  ...
  { name: 'Solar Arboretum', goal: 'Sync planting cycles with radiation waves; harness stellar energy.', reward: 'Prism Orchid unlocked', hazard: 'Solar flares imminent' },
];

const solutions = { 0: 'rewind', 1: 'fast', 2: 'hybrid', 3: 'prism' };
...
function DatapadFrame({ children, onClose }) {
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-black/50 p-4 backdrop-blur-sm">
      <div className="relative w-full max-h-[90vh] max-w-3xl overflow-hidden rounded-[2rem] border border-border bg-background shadow-lg">
        <button type="button" onClick={onClose} className="absolute right-4 top-4 z-10 grid h-10 w-10 place-items-center rounded-xl border border-border bg-card transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]">
  ...
  return (
    <div className="absolute inset-0 z-20 grid place-items-center rounded-[2rem] bg-background/75 p-4 backdrop-blur-sm">
      <div className="relative aspect-square w-[min(82vw,24rem)] rounded-full border border-primary/40 bg-card/95 shadow-lg">
        ...
        {petals.map(({ seed, x, y }) => (
          <button key={seed.id} type="button" onPointerDown={() => onSelect(seed.id)} onPointerUp={() => onSelect(seed.id)} aria-label={`Select ${seed.name}`} className={`absolute grid place-items-center rounded-[45%_55%_55%_45%] border bg-secondary text-center shadow-sm transition hover:scale-110 hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-95 ${selected === seed.id ? 'ring-2 ring-primary' : ''}`} style={{ left: `${x}%`, top: `${y}%`, rotate: `${(x - 50) / 2}deg`, touchAction: 'manipulation', width: '44px', height: '44px', minWidth: '44px', minHeight: '44px', transform: 'translate(-50%, -50%)' }}>
      ...
      <div className="grid min-h-72 place-items-center rounded-xl border border-primary/30 bg-muted/50 p-6">
        <div className="relative h-52 w-full max-w-2xl [transform:perspective(600px)_rotateX(58deg)_rotateZ(-4deg)]">
          ...
          {SECTORS.map((item, index) => {
            const cleared = index < sector || sector === 3;
            return <div key={item.name} className={`absolute grid h-20 w-32 place-items-center rounded-lg border ${cleared ? 'border-success bg-success-muted text-success' : index === sector ? 'border-warning bg-warning-muted text-warning' : 'border-border bg-card text-muted-foreground'}`} style={{ left: `${12 + index * 20}%`, top: `${30 + (index % 2) * 18}%` }}><span className="font-note text-base">{item.name}</span><span className="font-mono text-[8px] uppercase tracking-widest">{cleared ? 'cleared' : index === sector ? 'blight' : 'locked'}</span></div>;
          })}
        </div>
      </div>
...
export default function Garden() {
  ...
  function plant(seedId) {
    ...
    if (answer === seedId) {
      setMessage(sector === 0 ? 'REWIND accepted. The bridge remembers its shape and vines unfurl across the gap.' : sector === 1 ? 'FAST-FORWARD accepted. Oxidation races through the reinforced bulkhead.' : sector === 2 ? 'HYBRID accepted. Opposing temporal vectors braid around the station core.' : 'PRISM accepted. Light refracts through alien crystalline structures, revealing sealed passages.');
    ...
    <div className="min-h-[calc(100vh-73px)] bg-background">
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 md:px-7 md:py-7">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-border pb-5">
          ...
          <div className="flex items-center gap-2"><span className={`rounded-full border px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-widest ${frost ? 'border-warning bg-warning-muted text-warning' : 'border-success-border bg-success-muted text-success'}`}>{frost ? 'O₂ / frost warning' : current.hazard}</span><button type="button" onPointerDown={() => setDatapad('map')} onPointerUp={() => setDatapad('map')} aria-label="Open botanist datapad" className="grid place-items-center rounded-xl border border-border bg-card transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]" style={{ width: '44px', height: '44px', minWidth: '44px', minHeight: '44px' }}><ApperIcon name="TabletSmartphone" size={17} /></button><button type="button" onPointerDown={() => reset()} onPointerUp={() => reset()} aria-label="Reset garden" className="grid place-items-center rounded-xl border border-border bg-card transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]" style={{ width: '44px', height: '44px', minWidth: '44px', minHeight: '44px' }}><ApperIcon name="RotateCcw" size={16} /></button></div>
        </header>
        ...
        <main className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
          <section className="relative overflow-hidden rounded-[2rem] border border-border bg-card p-4 shadow-md md:p-6">
            ...
            <div className="relative mt-5 overflow-hidden rounded-[1.5rem] border border-border bg-muted p-3">
              <div className="grid grid-cols-4 gap-2 sm:grid-cols-6">
                {Array.from({ length: 24 }, (_, index) => {
                  ...
                  const radiant = index === 11 && sector === 3;
                  const solvedTile = (blocked && selected === 'rewind') || (barrier && selected === 'fast') || (core && solved) || (radiant && selected === 'prism');
                  return <div key={index} className={`relative grid aspect-square place-items-center overflow-hidden rounded-lg border bg-background text-xl transition ${blocked ? 'border-accent ring-2 ring-accent/40' : barrier ? 'border-warning ring-2 ring-warning/30' : core ? 'border-primary ring-2 ring-primary/30' : radiant ? 'border-success ring-2 ring-success/30' : 'border-border'}`}><span className="absolute inset-0 opacity-40" style={{ backgroundImage: 'radial-gradient(circle, var(--border) 1px, transparent 1px)', backgroundSize: '13px 13px' }} />{blocked ? (solvedTile ? '🌉' : '🕳️') : barrier ? (solvedTile ? '🪴' : '🧱') : core ? (solvedTile ? '🌳' : '◌') : radiant ? (solvedTile ? '💫' : '☀️') : index % 5 === 0 ? '🌿' : ''}</div>;
                })}
              </div>
            ...
            <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex-1 rounded-xl border border-border bg-background px-4 py-3"><p className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">Station telemetry</p><p className="mt-1 text-sm leading-relaxed text-foreground/80">{message}</p></div><button type="button" onPointerDown={() => setPodOpen(true)} onPointerUp={() => setPodOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground transition hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]" style={{ minHeight: '44px' }}><ApperIcon name="Flower2" size={17} /> Open Seed Pod</button></div>
          </section>

          <aside className="space-y-4">
            <section className="rounded-2xl border border-border bg-card p-4 shadow-xs"><div className="flex items-center justify-between"><h2 className="font-heading text-lg font-semibold">Botanist's datapad</h2><span className="font-mono text-[9px] text-muted-foreground">LIVE</span></div><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onPointerDown={() => setDatapad('map')} onPointerUp={() => setDatapad('map')} className="rounded-xl border border-border bg-background p-3 text-left transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]"><ApperIcon name="Map" size={16} /><p className="mt-2 font-mono text-[9px] uppercase tracking-wider">Map</p></button><button type="button" onPointerDown={() => setDatapad('flora')} onPointerUp={() => setDatapad('flora')} className="rounded-xl border border-border bg-background p-3 text-left transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]"><ApperIcon name="BookOpen" size={16} /><p className="mt-2 font-mono text-[9px] uppercase tracking-wider">Flora</p></button><button type="button" onPointerDown={() => setDatapad('splice')} onPointerUp={() => setDatapad('splice')} className="rounded-xl border border-border bg-background p-3 text-left transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring active:scale-[0.98]"><ApperIcon name="Dna" size={16} /><p className="mt-2 font-mono text-[9px] uppercase tracking-wider">Splice</p></button></div></section>
            ...
            <section className="rounded-2xl border border-border bg-card p-4"><div className="flex items-center justify-between"><p className="font-mono text-[9px] uppercase tracking-widest text-muted-foreground">Restoration</p><span className="font-mono text-[9px] text-muted-foreground">{Math.min(sector + (bred ? 1 : 0), 4)} / 4</span></div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-muted"><div className="h-full rounded-full bg-primary transition-all duration-(--transition-normal)" style={{ width: `${Math.min((sector + (bred ? 1 : 0)) / 4, 1) * 100}%` }} /></div><p className="mt-3 font-note text-lg">Next: {current.reward}</p></section>
          </aside>
        </main>
      </div>
